describe("avoid_error", () => {
  ...
  it("creates driver", () => {
    ...
    cy.chooseTeam("Testy automatyczne").then(() => {
      ...
      //OD TEGO MOMENTU NIE SPRAWDZANE
      cy.get("#spop--top-right").contains("Kierowca stworzony").should("be.visible")
      cy.get("tr")
        .contains(testname + " " + testname1)
        .parent()
        .invoke("text")
        .then((text) => {
          ...
          cy.get('a[href="/admin/partners/' + ID + '/edit"')
            ...
            .click()
          cy.get(".m-subheader__title").should("have.html", "Edycja kierowcy")
          cy.get('[data-original-title="Wyłącz"]').click({ force: true })

          cy.get("#drivers-active-button").click()
          cy.get('a[href="/admin/partners/' + ID + '/edit"')
            .contains("Edytuj dane")
            .click({ force: true })
          cy.get(".m-subheader__title").should("have.html", "Edycja kierowcy")
          cy.get('[data-original-title="Włącz"]').click({ force: true })
          cy.get(".m-subheader__title").should("have.html", "Kurierzy")
        })
    })
  })
})
